...
(async () => {
  ...
  const attrs = await page.locator('textarea[name="description"]').evaluate(e => ({
    readOnly: e.readOnly, disabled: e.disabled, maxLength: e.maxLength, value: e.value
  }));
  console.log('attrs', attrs);

  await page.locator('textarea[name="description"]').type('teste desc', { delay: 30 });
  await page.waitForTimeout(200);
  const val2 = await page.locator('textarea[name="description"]').inputValue();
